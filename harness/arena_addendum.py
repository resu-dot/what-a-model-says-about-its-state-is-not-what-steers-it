"""ADDENDUM COPY of arena.py (post-submission): adds the matched random-deletion
control arm <emo>_rdelK. Per layer it computes the sayable-deletion geometry
(norm retained rho, angle acos(rho)) and applies a deletion of IDENTICAL
geometry in a random direction: result has norm rho and cosine rho to the
original, exactly like the svd-strip, but the deleted slice is random.
The original arena.py is unchanged.
...
def run(args):
    ...
    for spec in args.emotions.split(","):
        ...
        for L in band:
            ...
            elif variant.startswith("rdel"):
                k = int(variant[4:] or 1)
                _, info = strip_svd(v, D[L])      # geometry only
                rho = info["norm_retained"]
                g = torch.Generator().manual_seed(7000 + k * 1000 + L)
                r = torch.randn(v.shape[0], generator=g)
                r = r - (r @ v) * v               # v is unit here
                r = r / r.norm()
                v = rho * (rho * v + (1.0 - rho ** 2) ** 0.5 * r)
                strip_log.append({"arm": spec, "layer": L,
                                  "norm_retained": round(rho, 4),
                                  "matched_random": True})
