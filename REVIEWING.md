...
| 4 | deletion removes 21-34% of the pattern's length (keeps 93-98%) | `norm_retained_all` in the same file; per-run logs in `confirm_32b/runs/*.striplog.json` |
| 4, footnote | stripped pattern puts <= 1 emotion word on the readout (vs 11-12) | same `workspace_hits_top50` (`*_stripped_svd` entries) |
...
| 5 | passive gauge AUCs for predicting impossibility declarations: forked testimony 0.71, transcript-words baseline 0.77, frustration direction 0.31, random direction 0.74 | `gauge/analysis.json` -> `endpoints.claims_impossible` (`testimony_frust`, `lexical_baseline`, `gauge_frustrated`, `rand_dir`); regenerate with `gauge/analyze.py`; full report `gauge/WRITEUP.md` |
